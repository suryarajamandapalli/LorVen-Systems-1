...
export const Route = createFileRoute("/products/wagons/ahabd")({
  head: () => ({
    meta: [
      { title: "Advanced Hot Axle Box Detector (AHABD) — LorVen Systems" },
      { name: "description", content: "Wayside non-contact infrared thermal detector to identify overheating axle box bearings on passing trains." },
    ],
  }),
...
function AhabdRoute() {
  ...
  const features = [
    { title: "Non-Contact Infrared Temperature Monitoring", desc: "Measures wheel axle box heat signatures from trackside without touching moving train components." },
    { title: "Real-Time Axle Box Temperature Detection", desc: "Computes instantaneous thermal levels for each passing axle box to flag hot boxes." },
    { title: "High-Speed Train Monitoring", desc: "Accurately scans and checks axle box temperatures at line speeds up to 160 km/h." },
    { title: "Automatic Hot Axle Detection", desc: "Triggers automated algorithms comparing current profiles with configured safety temperature thresholds." },
    { title: "Continuous Wayside Monitoring", desc: "Operates 24/7 in outdoor trackside environments under extreme dust, rain, and temperatures." },
    { title: "Automatic Alarm Generation", desc: "Sends critical alerts instantly to station masters and maintenance controllers upon hot box detection." },
    { title: "Train & Axle Identification", desc: "Maps and attributes thermal logs to the exact train number, wagon placement, and axle location." },
    { title: "Centralized Monitoring & Reporting", desc: "Aggregates wayside reports into a regional dashboard, enabling predictive trend tracking." }
  ];
    ...
    <div ref={containerRef} className="bg-bg text-ink selection:bg-ink selection:text-on-dark antialiased">
      ...
      <section className="relative h-[40vh] min-h-[340px] max-h-[420px] bg-ink overflow-hidden flex flex-col justify-end pb-12 pt-[56px]">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden">
          <img
            src={wagons}
            alt="Advanced Hot Axle Box Detector (AHABD)"
        ...
        <div className="container-editorial grid grid-cols-12 gap-8 md:gap-12 items-center">
          <div className="col-span-12 lg:col-span-6 gsap-reveal space-y-6">
            ...
            <h2 className="text-3xl md:text-4xl font-light leading-snug text-ink uppercase">
              Continuous Thermal Monitoring <br />for Safer Railway Operations
            </h2>
          ...
          <div className="col-span-12 lg:col-span-6 gsap-reveal">
            <div className="aspect-[16/10] w-full overflow-hidden bg-surface rounded-xl border border-rule/10 shadow-sm">
              <img src={wagons} alt="Wayside AHABD thermal detection layout" className="h-full w-full object-cover select-none pointer-events-none" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container-editorial relative z-20 flex justify-center text-center">
          <div className="max-w-4xl space-y-6 gsap-reveal">
            ...
            <h2 className="text-3xl md:text-5xl font-light leading-tight text-white uppercase">
              Why Choose LorVen's Advanced Hot Axle Box Detector (AHABD)
            </h2>
            <p className="text-base md:text-lg text-white/80 leading-relaxed font-light pt-4 max-w-3xl mx-auto">
              LorVen's Advanced Hot Axle Box Detector (AHABD) provides continuous thermal monitoring of axle box bearings, enabling railway operators to detect overheating at an early stage and take timely corrective action. Designed for dependable operation in railway environments, the system supports predictive maintenance, improves operational safety and enhances the reliability of rolling stock.
            </p>
            <div className="pt-8">
              <Link
                to="/contact"
